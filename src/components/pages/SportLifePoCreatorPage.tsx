...
const SportLifePoCreatorPage = ({ pageId }: { pageId: string }) => {
  ...
  const addMockItems = useCallback(async (userId: string) => {
    ...
    try {
      ...
      const mockItems = [
        ...
        { name: 'SportLife Yoga Mat (Blue)', wholesaleCost: 18.00 },
        { name: 'SportLife Water Bottle (1L)', wholesaleCost: 10.00 },
        { name: 'SportLife Creatine Monohydrate', wholesaleCost: 15.75 },
      ];
      ...
      await batch.commit();
      console.log("Mock items added to Firestore.");
    } catch (e) {
  ...
  const addMockCustomers = useCallback(async (userId: string) => {
    ...
    try {
      ...
      await batch.commit();
      console.log("Mock customers added to Firestore.");
    } catch (e) {
  ...
  useEffect(() => {
    ...
    const unsubscribeCustomers = onSnapshot(customersColRef, async (snapshot) => {
      ...
      if (snapshot.docs.length === 0) {
        ...
        const journeyCustomer = customersData.find(c => c.isDefault);
        if (journeyCustomer && !selectedCustomerId) {
  ...
  const calculateCurrentOrderTotal = useCallback(() => {
    ...
    for (const itemId in currentOrderQuantities) {
      ...
      if (item) {
        subtotal += item.wholesaleCost * (currentOrderQuantities[itemId] || 0);
      }
    }
    return subtotal;
  ...
  const calculateAggregatedPOTotal = useCallback(() => {
    let subtotal = 0;
    const compiledItemsMap: Record<string, PoItem & { originalItemIds?: string[] }> = {};
    ...
    Object.values(compiledItemsMap).forEach(item => {
        subtotal += item.wholesaleCost * item.quantity;
    });
    
    const processingFeeAmount = subtotal * (processingFeePercentage / 100);
    return {
      compiledItems: Object.values(compiledItemsMap),
      subtotal: subtotal,
      processingFee: processingFeeAmount,
      grandTotal: (subtotal + processingFeeAmount)
    };
  ...
  const handleAddItem = async (newItemData: { name: string, wholesaleCost: number }) => {
    ...
    try {
      ...
      showAppToast("Error", "Failed to add item.", "destructive");
    }
  };
  ...
  const handleEditItem = async (editedItemData: SportLifeItemDef) => {
    ...
    try {
      ...
      showAppToast("Error", "Failed to update item.", "destructive");
    }
  };
  ...
  const handleAddCustomer = async (newCustomerData: { firstName: string, lastName: string }) => {
    ...
    try {
      ...
      showAppToast("Error", "Failed to add customer.", "destructive");
    }
  };

  // Handlers for Individual Order (PO) Management
  const handleSaveOrder = async () => {
    ...
    try {
      if (editingPoId) {
        ...
        if (!querySnapshot.empty) {
            ...
            orderItems.forEach(newItem => {
                if (mergedItemsMap[newItem.itemId]) {
                    mergedItemsMap[newItem.itemId].quantity += newItem.quantity;
                } else {
                    mergedItemsMap[newItem.itemId] = { ...newItem };
                }
            });
            ...
            const consolidatedPoData = {
                ...existingData,
                items: Object.values(mergedItemsMap),
                subtotal: newConsolidatedSubtotal,
                orderDate: new Date().toISOString(),
            };
      ...
      setCurrentOrderQuantities({});
      setSelectedCustomerId('');
      setEditingPoId(null);
    } catch (e) {
      console.error("Error saving/updating order: ", e);
      showAppToast("Error", "Failed to save order.", "destructive");
    }
  };

  const handleEditSavedOrder = (po: PurchaseOrderDef) => {
    setEditingPoId(po.id);
    setSelectedCustomerId(po.customerId);
    const quantities: Record<string, number> = {};
    po.items.forEach(item => { quantities[item.itemId] = item.quantity; });
    setCurrentOrderQuantities(quantities);
    setIsEditingSavedOrders(false);
  ...
  const handleDeleteSavedOrder = async (poId: string) => {
    ...
    try {
      ...
      showAppToast("Error", "Failed to delete saved order.", "destructive");
    }
  };


  // Handler for Creating Final PO Preview
  const handleCreateFinalPoPreview = useCallback(() => {
    const savedOrdersToCompile = purchaseOrders.filter(po => po.status === 'saved');
    if (savedOrdersToCompile.length === 0) {
      showAppToast("Info", "No saved orders to compile.");
    ...
    if (subtotal < minOrder) {
      showAppToast("Minimum Order Not Met", `Combined wholesale total is $${subtotal.toFixed(2)}. Vendor minimum is $${minOrder.toFixed(2)}. Please add more items.`, "destructive");
      setFinalPoDraft(null);
      return;
    }
  ...
  // Handler for Actual Final PO Submission
  const handleSubmitActualPo = async () => {
    if (!db || !currentUser || !finalPoDraft) {
      showAppToast("Error", "Cannot submit PO. Draft or database not ready.", "destructive");
      return;
    }
    setIsSubmitting(true);
    const savedOrdersToSubmit = purchaseOrders.filter(po => po.status === 'saved');
    try {
      const batch = writeBatch(db);
      savedOrdersToSubmit.forEach(order => {
        ...
        batch.update(orderRef, {
          ...
          finalTotal: individualOrderGrandTotal
        });
      });
      await batch.commit();
      ...
      console.error("Error submitting final PO: ", e);
      showAppToast("Error", "Failed to submit final Purchase Order.", "destructive");
    } finally {
  ...
  if (isLoadingData) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        ...
        <p className="ml-4 text-slate-600">Loading SportLife PO System...</p>
      </div>
    );
  }
  ...
  const AddEditItemForm = ({ item, onSubmit }: { item: SportLifeItemDef | null, onSubmit: (data: any) => void }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
      ...
      if (!name || isNaN(cost) || cost <= 0) {
        showAppToast("Validation Error", "Please enter a valid item name and positive wholesale cost.", "destructive");
        return;
      }
    ...
    return (
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <Label htmlFor="itemNamePoForm">Item Name</Label>
          <Input id="itemNamePoForm" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g., Protein Powder" required />
        </div>
        <div>
          <Label htmlFor="wholesaleCostPoForm">Wholesale Cost ($)</Label>
          <Input id="wholesaleCostPoForm" type="number" value={wholesaleCost} onChange={(e) => setWholesaleCost(e.target.value)} placeholder="e.g., 28.50" step="0.01" min="0.01" required />
        </div>
        <Button type="submit" className="w-full">
          {item ? <><Save className="mr-2 h-4 w-4" />Save Item Changes</> : <><PlusCircle className="mr-2 h-4 w-4" />Save New Item</>}
        </Button>
      </form>
    );
  };
  ...
  const AddCustomerForm = ({ onSubmit }: { onSubmit: (data: any) => void }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
      ...
      if (!firstName || !lastName) {
        showAppToast("Validation Error", "Please enter first and last name.", "destructive");
        return;
      }
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-8">
      <Card>
        <CardHeader className="text-center">
          <CardTitle className="text-3xl md:text-4xl font-bold tracking-tight text-slate-900 flex items-center justify-center gap-3">
      ...
      {/* Current Order Creation Section */}
      <Card>
        <CardHeader>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            <CardTitle className="text-2xl font-bold text-slate-800 flex items-center gap-2">
              <PlusCircle className="text-sky-600 w-7 h-7" />
              {editingPoId ? 'Edit Current Order' : 'Create New Order'}
            </CardTitle>
            <Button onClick={() => setIsEditingItems(!isEditingItems)} variant="outline" size="sm">
              <Edit className="mr-2 h-4 w-4" /> {isEditingItems ? 'Done Editing Items' : 'Edit Items Catalog'}
            </Button>
          </div>
        </CardHeader>
        <CardContent className="space-y-6">
                    ...
                    <TableRow key={item.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1">
                          <Button variant="outline" size="icon" className="h-7 w-7" onClick={() => handleAdjustQuantity(item.id, -1)}><Minus className="h-4 w-4" /></Button>
                          <Input type="number" value={currentOrderQuantities[item.id] || ''} onChange={(e) => handleQuantityChange(item.id, e.target.value)} onFocus={(e) => e.target.select()} className="w-16 text-center h-7 px-1"/>
                          <Button variant="outline" size="icon" className="h-7 w-7" onClick={() => handleAdjustQuantity(item.id, 1)}><Plus className="h-4 w-4" /></Button>
                        </div>
                      </TableCell>
                      <TableCell className="font-semibold">${calculateLineTotal(item).toFixed(2)}</TableCell>
                      {isEditingItems && (
                        <TableCell className="space-x-1">
                          <Button variant="ghost" size="icon" className="h-7 w-7 text-sky-600 hover:bg-sky-100" onClick={() => { setItemToEdit(item); setShowAddItemModal(true); }}><Edit className="h-4 w-4" /></Button>
                          <Button variant="ghost" size="icon" className="h-7 w-7 text-red-500 hover:bg-red-100" onClick={() => handleDeleteItem(item.id)}><Trash2 className="h-4 w-4" /></Button>
                        </TableCell>
                      )}
                    </TableRow>
                  ))}
          ...
          {isEditingItems &&
            <Button onClick={() => { setItemToEdit(null); setShowAddItemModal(true); }} variant="default" size="sm">
                <PlusCircle className="mr-2 h-4 w-4" /> Add New Item to Catalog
            </Button>
          }
          ...
          <Card className="bg-slate-50/50 p-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
              <div>
                <Label htmlFor="customerSelectPoForm">Select Customer for this Order</Label>
                <div className="flex gap-2">
                    <Select value={selectedCustomerId} onValueChange={setSelectedCustomerId}>
                        <SelectTrigger id="customerSelectPoForm" className="flex-grow"><SelectValue placeholder="Select a customer" /></SelectTrigger>
                        <SelectContent>
                            <SelectItem value="">Select a customer</SelectItem>
                            {customers.map(customer => <SelectItem key={customer.id} value={customer.id}>{customer.firstName} {customer.lastName}</SelectItem>)}
                        </SelectContent>
                    </Select>
                    <Button variant="outline" size="icon" onClick={() => setShowAddCustomerModal(true)} aria-label="Add new customer" className="flex-shrink-0"><UserPlus className="h-4 w-4"/></Button>
                </div>
              </div>
              <div className="text-right">
                <p className="text-md font-semibold text-slate-700">Current Order Subtotal: <span className="text-lg text-slate-900">${calculateCurrentOrderTotal().toFixed(2)}</span></p>
              </div>
            </div>
          </Card>
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            <CardTitle className="text-2xl font-bold text-slate-800 flex items-center gap-2">
              <Package className="text-teal-600 w-7 h-7" /> Saved Orders
            </CardTitle>
            {purchaseOrders.filter(po => po.status === 'saved').length > 0 && (
                <Button onClick={() => setIsEditingSavedOrders(!isEditingSavedOrders)} variant="outline" size="sm">
                    <Edit className="mr-2 h-4 w-4" /> {isEditingSavedOrders ? 'Done Editing Orders' : 'Edit Saved Orders'}
                </Button>
            )}
          </div>
        </CardHeader>
        <CardContent>
          {purchaseOrders.filter(po => po.status === 'saved').length === 0 ? (
            <p className="text-slate-500 text-center py-4">No orders currently saved. Create one above to get started.</p>
          ) : (
            <div className="space-y-4">
              {purchaseOrders.filter(po => po.status === 'saved').map(po => (
                <Card key={po.id} className="bg-white">
                  <CardHeader className="pb-3 pt-4 px-4">
                    <div className="flex justify-between items-center">
                      <div>
                        <CardTitle className="text-lg font-semibold text-slate-800">{po.customerName}</CardTitle>
                        <CardDescription className="text-xs text-slate-500">Saved: {new Date(po.orderDate).toLocaleDateString()}</CardDescription>
                      </div>
                      {isEditingSavedOrders && (
                        <div className="flex gap-1">
                            <Button variant="outline" size="sm" onClick={() => handleEditSavedOrder(po)}><Edit className="mr-1 h-3 w-3"/> Edit</Button>
                            <Button variant="destructive" size="sm" onClick={() => handleDeleteSavedOrder(po.id)}><Trash2 className="mr-1 h-3 w-3"/> Delete</Button>
                        </div>
                      )}
                    </div>
                  </CardHeader>
          ...
          {purchaseOrders.filter(po => po.status === 'saved').length > 0 && (
            <div className="mt-6 pt-6 border-t flex justify-end">
              <Button onClick={handleCreateFinalPoPreview} size="lg" variant="default">
                <CheckCircle2 className="mr-2 h-5 w-5" /> Compile All Saved Orders into PO Draft
              </Button>
            </div>
          )}
        </CardContent>
      ...
      {/* Final PO Draft Section */}
      {finalPoDraft && (
        <Card>
          <CardHeader>
            <CardTitle className="text-2xl font-bold text-slate-800 flex items-center gap-2">
              <DollarSign className="text-blue-600 w-7 h-7" /> SportLife PO Draft
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="overflow-x-auto rounded-lg border">
              <Table><TableHeader><TableRow><TableHead>Item Name</TableHead><TableHead>Total Qty</TableHead><TableHead>Unit Cost</TableHead><TableHead className="text-right">Total Cost</TableHead></TableRow></TableHeader>
                <TableBody>
                  {finalPoDraft.compiledItems.map((item, idx) => (
                    <TableRow key={idx}><TableCell className="font-medium">{item.name}</TableCell><TableCell>{item.quantity}</TableCell><TableCell>${item.wholesaleCost.toFixed(2)}</TableCell><TableCell className="text-right font-semibold">${(item.wholesaleCost * item.quantity).toFixed(2)}</TableCell></TableRow>
                  ))}
                </TableBody>
              </Table>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end p-4 bg-slate-50/50 rounded-lg border">
              <div>
                <Label htmlFor="processingFeePoForm">Global Processing Fee (%)</Label>
                <div className="flex items-center">
                <Input id="processingFeePoForm" type="number" value={processingFeePercentage} onChange={(e) => setProcessingFeePercentage(Math.max(0, parseFloat(e.target.value) || 0))} className="w-24 text-right" step="0.1" min="0"/>
                <span className="ml-2 text-slate-600">%</span>
                </div>
              </div>
              <div className="text-right space-y-1">
                <p className="text-md font-semibold text-slate-700">Aggregated Subtotal: <span className="text-lg text-slate-900">${finalPoDraft.subtotal.toFixed(2)}</span></p>
                <p className="text-md font-semibold text-slate-700">Processing Fee ({finalPoDraft.processingFeePercentage}%): <span className="text-lg text-slate-900">${finalPoDraft.processingFee.toFixed(2)}</span></p>
                <p className="text-xl font-bold text-indigo-700">Grand Total: <span className="text-2xl">${finalPoDraft.grandTotal.toFixed(2)}</span></p>
              </div>
            </div>
            <div className="flex justify-end">
              <Button onClick={handleSubmitActualPo} size="lg" variant="default" disabled={isSubmitting}>
                {isSubmitting ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : <CheckCircle2 className="mr-2 h-5 w-5" />}
                {isSubmitting ? 'Submitting...' : 'Submit Final PO to SportLife'}
              </Button>
            </div>
          </CardContent>
        </Card>
      ...
      <Dialog open={showAddItemModal} onOpenChange={(open) => { if(!open) { setShowAddItemModal(false); setItemToEdit(null);}}}>
        <DialogContent><DialogHeader><DialogTitle>{itemToEdit ? 'Edit SportLife Item' : 'Add New SportLife Item'}</DialogTitle></DialogHeader>
          <AddEditItemForm item={itemToEdit} onSubmit={itemToEdit ? handleEditItem : handleAddItem} />
        </DialogContent>
      </Dialog>
      <Dialog open={showAddCustomerModal} onOpenChange={setShowAddCustomerModal}>
        <DialogContent><DialogHeader><DialogTitle>Add New Customer</DialogTitle></DialogHeader>
          <AddCustomerForm onSubmit={handleAddCustomer} />
        </DialogContent>
      </Dialog>
      
      <Card className="mt-8">
        <CardHeader><CardTitle className="text-lg">Submitted Orders</CardTitle></CardHeader>
        <CardContent>
             {purchaseOrders.filter(po => po.status === 'submitted').length === 0 ? (
                <p className="text-slate-500 text-center py-4">No orders have been submitted yet.</p>
            ) : (
                <div className="space-y-4">
                {purchaseOrders.filter(po => po.status === 'submitted').map(po => (
                    <Card key={po.id} className="bg-green-50/70 border-green-200">
                    <CardHeader className="pb-2 pt-3 px-4">
                        <CardTitle className="text-md font-semibold text-green-800">{po.customerName}</CardTitle>
                        <CardDescription className="text-xs text-green-600">Submitted: {new Date(po.orderDate).toLocaleString()}</CardDescription>
                    </CardHeader>
                    <CardContent className="px-4 pb-3 text-xs">
                        <ul className="list-disc list-inside space-y-0.5 text-slate-600">
                        {po.items.map((item, idx) => <li key={idx}>{item.name} (Qty: {item.quantity})</li>)}
                        </ul>
                        <p className="mt-1 pt-1 border-t border-green-100 text-right">
                           <span className="font-medium">Subtotal: ${po.subtotal.toFixed(2)}</span><br/>
                           {po.processingFeePercentage !== undefined && <span className="text-slate-500">Fee ({po.processingFeePercentage}%): ${(po.subtotal * (po.processingFeePercentage/100)).toFixed(2)}</span>}<br/>
                           {po.finalTotal !== undefined && <span className="font-bold text-green-700">Final Total: ${po.finalTotal.toFixed(2)}</span>}
                        </p>
                    </CardContent>
                    </Card>
                ))}
