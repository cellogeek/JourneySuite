...
import { PlusCircle, Trash2, Beaker, RotateCcw } from 'lucide-react';
...
const initialSandboxItems: SandboxItem[] = [
  { id: 'sb-1', name: 'Experimental Coffee Blend', quantity: 10, unit: 'kg' },
  { id: 'sb-2', name: 'Test Batch Pastries', quantity: 24, unit: 'pcs' },
  { id: 'sb-3', name: 'Sample Syrup Flavor', quantity: 5, unit: 'bottles' },
];

const InventorySandboxPage = ({ pageId }: { pageId: string }) => {
  ...
  const handleAddItem = () => {
    ...
    toast({
      title: "Item Added",
      description: `"${newSandboxItem.name}" has been added to the sandbox.`,
    });
  };
  ...
  const handleResetSandbox = () => {
    ...
    toast({
      title: "Sandbox Reset",
      description: "The inventory sandbox has been reset to its initial state.",
    });
  };
  ...
  return (
    <div className="space-y-8">
      <Card className="content-fade-in-up">
        <CardHeader>
          <CardTitle className="text-3xl md:text-4xl font-bold tracking-tight text-slate-900 flex items-center">
            <Beaker size={32} className="mr-3 text-purple-600" /> Inventory Sandbox
          </CardTitle>
          <CardDescription className="text-lg text-slate-500 pt-1">
            Experiment with inventory items without affecting production data.
          </CardDescription>
        </CardHeader>
        <CardContent>
            <Button onClick={handleResetSandbox} variant="outline" size="sm">
      ...
      <Card className="content-fade-in-up" style={{ animationDelay: '150ms' }}>
        <CardHeader>
          <CardTitle>Add New Sandbox Item</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card className="content-fade-in-up" style={{ animationDelay: '300ms' }}>
        <CardHeader>
          <CardTitle>Current Sandbox Stock</CardTitle>
        </CardHeader>
        <CardContent>
                  ...
                  <TableRow key={item.id}>
                    ...
                    <TableRow>
                        <TableCell colSpan={4} className="text-center h-24 text-slate-500">
                            The sandbox is empty. Add an item to get started.
                        </TableCell>
                    </TableRow>
                )}
              </TableBody>
...
export default InventorySandboxPage;
